...
{
    T ratio = atan(sqrt(1. - e)*sin(theta / 2.)/(sqrt(1. + e) * cos(theta / 2.)));
    if ( DACE::cons((sqrt(1. + e) * cos(theta / 2.))) < 0.0 ) { ratio += M_PI; }
    if ( DACE::cons(ratio) < 0.0 ) { ratio += 2.0*M_PI; } //only to have alpha \in [0,2pi]
...
{
     T ratio = atan(sqrt(1. + e)*sin(E / 2.)/(sqrt(1. - e) * cos(E / 2.)));
    if ( DACE::cons(sqrt(1. - e) * cos(E / 2.)) < 0.0 ) { ratio += M_PI; }
    if ( DACE::cons(ratio) < 0.0 ) { ratio += 2.0*M_PI; } //only to have alpha \in [0,2pi]
...
template<typename T> AlgebraicVector<T> cart2kep(const AlgebraicVector<T>& rv, const double mu)
{
	...
	{
		e = 0.0;
        omega = atan(rr[1]/rr[0]);
        if ( DACE::cons(rr[0]) < 0.0 ) { omega += M_PI; }
        if ( DACE::cons(omega) < 0.0 ) { omega += 2.0*M_PI; } //only to have alpha \in [0,2pi]
	...
	if (cons(e) <= 1.0e-8 && cons(i) >= 1.0e-8)
	{
        ...
        theta = atan(rrt[1]/rrt[0]);
        if ( DACE::cons(rrt[0]) < 0.0 ) { theta += M_PI; }
        if ( DACE::cons(theta) < 0.0 ) { theta += 2.0*M_PI; } //only to have alpha \in [0,2pi]
	...
	if (cons(i) <= 1.0e-8 && cons(e) >= 1.0e-8)
	{
        ...
        omega = atan(ee[1]/ee[0]);
        if ( DACE::cons(ee[0]) < 0.0 ) { omega += M_PI; }
        if ( DACE::cons(omega) < 0.0 ) { omega += 2.0*M_PI; } //only to have alpha \in [0,2pi]
